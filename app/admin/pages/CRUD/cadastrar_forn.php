...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    function formatarTelefone($telefone)
    {
        ...
        // Verifica se o número tem o tamanho correto (11 dígitos para telefone celular no Brasil)
        if (strlen($telefone) == 11) {
            // Formata o telefone (ex: (XX) XXXXX-XXXX)
            $telefoneFormatado = '(' . substr($telefone, 0, 2) . ') ' . substr($telefone, 2, 5) . '-' . substr($telefone, 7);
            return $telefoneFormatado;
        }
        return $telefone; // Caso não tenha 11 dígitos, retorna o telefone sem formatação
    }

    // Formata o telefone antes de salvar
    $telefoneFornecedor = formatarTelefone($telefoneFornecedor);
...
<head>
    ...
    <script>
        ...
        function formatarTelefone(telefone) {
            ...
            // Verifica o tamanho do telefone e aplica a formatação
            if (telefone.length <= 10) {
                // Formato (XX) XXXXX-XXXX
                telefone = telefone.replace(/^(\d{2})(\d{5})(\d{4})$/, '($1) $2-$3');
            } else {
                // Caso o número seja maior que 10 (DDI internacional)
        ...
        // Função que chama a formatação ao digitar
